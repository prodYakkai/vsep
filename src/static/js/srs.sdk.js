...
function SrsRtcWhipWhepAsync(
    ...
    // The callback when got local stream.
    // @see https://developer.mozilla.org/en-US/docs/Web/API/RTCPeerConnection/addStream#Migrating_to_addTrack
    self.ontrack = async function (event) {
        // Add track to stream of SDK.
        const mediaStreamTrack = event.track;
        if (mediaStreamTrack.kind === 'audio' && self.requestDevice) {
            mediaStreamTrack.applyConstraints({
                deviceId: await lookupDevice(self.requestDevice.deviceId, self.requestDevice.label).deviceId,
            });
        }
        self.stream.addTrack(mediaStreamTrack);
    };
    ...
    self.pc.addEventListener('track', (event) => {
